# PVQC 電機電子專業英文測驗挑戰系統 (793 題庫) `v2.3.0`
> **系統開發設計：劉建良@2026 免費軟體** ｜ 版本：`v2.3.0 (2026.09 手機版左右雙欄獨立滑動・繁體筆劃排序・即時搜尋強化版)`
...
## 🌟 核心特色與遊戲模式 (v2.3.0 升級版)

### 1. 🔀 打散代號配對模式 (手機左右雙欄獨立滑動・筆劃排序・即時搜尋)
* **📱 左右雙欄獨立滑動 (雙視窗並排)**：
  * 左側「英文題目欄」與右側「中文對照表」**各自獨立上下捲動**。
  * 手機與桌機皆不用在超長頁面上下來回狂滑！在左欄選題目、右欄滑動找中文，點擊即時秒配對。
  * 支援三大檢視模式快速切換：**「左右雙欄並排」**、**「僅看英文題目」**、**「僅看中文對照」**。
* **🔤 繁體中文首字筆畫排序**：
  * 原生支援台灣標準繁體筆劃排序演算法（2 畫「二極體」$\rightarrow$ 6 畫「安培」$\rightarrow$ 13 畫「電源」），查閱中文釋義直覺迅捷。
* **🔍 即時關鍵字搜尋過濾**：
  * 右欄常駐搜尋列，輸入「電」、「阻」、「計」等關鍵字或代號，即時動態過濾對照選項。
* **👁️ 只顯示未配對選項**：
  * 可一鍵切換「隱藏已配對選項」，隨著作答卡片自動縮減，越答越輕鬆！
* **全卷唯一編碼防呆**：
  * $\le 26$ 題採用英文字母 `A ~ Z`；$> 26$ 題自適應指派全卷唯一代號 `1 ~ N`，代號絕不重複碰撞。
* **未填答即時提醒與題號直達**：
  * 頂部動態產生「未作答題號按鈕雲（如 `#7`、`#14`）」，點擊任一題號立刻平滑滾動直達該題！
  * 題目旁邊常駐 `⚠️ 待作答` 與 `✅ 配對 [代號]` 狀態膠囊。
  * 交卷時若有遺漏，自動亮起橘黃色呼吸燈發光標記，並跳出未填題號提示框。
